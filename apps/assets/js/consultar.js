...
function doGet(e) {
  ...
  document.getElementById('myForm').addEventListener('submit', async function(event) {
    ...
    try {
        const response = await fetch('https://script.google.com/macros/s/AKfycbytD6we6zvOKY57p9rnZzbtpwSoKd8t3j0u99JpSmmdry_E0Pl9cUVscCWa-5S9yd_W/exec', {
            method: 'POST',
            headers: {
...
function doGet(e) {
  ...
  for (var i = 1; i < values.length; i++) {
    ...
    row['capa'] = values[i][0];
    row['capas'] = values[i][1];
    row['nomecapa'] = values[i][2];
    row['envio'] = values[i][3];
    row['pagamento'] = values[i][4];
    row['nomecliente'] = values[i][5];
    row['emailcliente'] = values[1][6];
    row['telefonecliente'] = values[1][7];

    saida.push(row);
  }
  if (equipe != null) {
    var retornoDaSaida = saida.filter(obj => obj.Equipe.toLowerCase().includes(equipe.toLowerCase()));
    return ContentService.createTextOutput(JSON.stringify({ retornoDaSaida })).setMimeType(ContentService.MimeType.JSON);
  }
  return ContentService.createTextOutput(JSON.stringify({ saida })).setMimeType(ContentService.MimeType.JSON);
}